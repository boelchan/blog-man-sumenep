...
class ArtikelDataTable extends DataTable
{
    public function dataTable($query)
    {
        ...
        return $dataTable->editColumn('published_at', function ($query) {
            ...
            ->editColumn('action', function ($query) {
                return view('components.button.show', ['action' => route('post.artikel.show', [$query->id, 'uuid' => $query->uuid])]).
                    view('components.button.edit', ['action' => route('post.artikel.edit', [$query->id, 'uuid' => $query->uuid])]).
                    view('components.button.destroy', ['action' => route('post.artikel.destroy', [$query->id, 'uuid' => $query->uuid]), 'label' => $query->judul, 'target' => 'artikel-table']);
            })
    ...
    {
        return $model->newQuery()->where('kategori_id', CategoryEnum::ARTIKEL);
    }
