...
class BannerDataTable extends DataTable
{
    public function dataTable($query)
    {
        ...
        return $dataTable->editColumn('published_at', function ($query) {
            ...
            ->editColumn('action', function ($query) {
                return view('components.button.show', ['action' => route('post.banner.show', [$query->id, 'uuid' => $query->uuid])]).
                    view('components.button.edit', ['action' => route('post.banner.edit', [$query->id, 'uuid' => $query->uuid])]).
                    view('components.button.destroy', ['action' => route('post.banner.destroy', [$query->id, 'uuid' => $query->uuid]), 'label' => $query->judul, 'target' => 'banner-table']);
            })
    ...
    {
        return $model->newQuery()->where('kategori_id', CategoryEnum::BANNER);
    }
