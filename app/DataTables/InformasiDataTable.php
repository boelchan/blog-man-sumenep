...
class InformasiDataTable extends DataTable
{
    public function dataTable($query)
    {
        ...
        return $dataTable->editColumn('published_at', function ($query) {
            ...
            ->editColumn('action', function ($query) {
                return view('components.button.show', ['action' => route('post.informasi.show', [$query->id, 'uuid' => $query->uuid])]).
                    view('components.button.edit', ['action' => route('post.informasi.edit', [$query->id, 'uuid' => $query->uuid])]).
                    view('components.button.destroy', ['action' => route('post.informasi.destroy', [$query->id, 'uuid' => $query->uuid]), 'label' => $query->judul, 'target' => 'informasi-table']);
            })
    ...
    {
        return $model->newQuery()->where('kategori_id', CategoryEnum::INFORMASI);
    }

    public function html()
    {
        return $this->builder()
            ->setTableId('informasi-table')
