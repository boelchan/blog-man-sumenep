    ...
    public function dataTable($query)
    {
        ...
        return $dataTable->editColumn('publish', function ($query) {
            ...
            ->editColumn('action', function ($query) {
                return view('components.button.show', ['action' => route('post.poli.show', [$query->id, 'uuid' => $query->uuid])]).
                    view('components.button.edit', ['action' => route('post.poli.edit', [$query->id, 'uuid' => $query->uuid])]).
                    view('components.button.destroy', ['action' => route('post.poli.destroy', [$query->id, 'uuid' => $query->uuid]), 'label' => $query->nama, 'target' => 'poli-table']);
            })
            ->rawColumns(['publish', 'action']);
    }

    public function query(Poly $model)
    {
