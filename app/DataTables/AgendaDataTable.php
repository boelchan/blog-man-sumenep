...
class AgendaDataTable extends DataTable
{
    public function dataTable($query)
    {
        ...
        return $dataTable->editColumn('published_at', function ($query) {
            ...
            ->editColumn('action', function ($query) {
                return view('components.button.show', ['action' => route('post.agenda.show', [$query->id, 'uuid' => $query->uuid])]).
                    view('components.button.edit', ['action' => route('post.agenda.edit', [$query->id, 'uuid' => $query->uuid])]).
                    view('components.button.destroy', ['action' => route('post.agenda.destroy', [$query->id, 'uuid' => $query->uuid]), 'label' => $query->judul, 'target' => 'agenda-table']);
            })
    ...
    {
        return $model->newQuery()->where('kategori_id', CategoryEnum::AGENDA);
    }
