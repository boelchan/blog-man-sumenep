...
class GalleryDataTable extends DataTable
{
    public function dataTable($query)
    {
        ...
        return $dataTable->editColumn('published_at', function ($query) {
            ...
            ->editColumn('action', function ($query) {
                return view('components.button.show', ['action' => route('post.gallery.show', [$query->id, 'uuid' => $query->uuid])]).
                    view('components.button.edit', ['action' => route('post.gallery.edit', [$query->id, 'uuid' => $query->uuid])]).
                    view('components.button.destroy', ['action' => route('post.gallery.destroy', [$query->id, 'uuid' => $query->uuid]), 'label' => $query->judul, 'target' => 'gallery-table']);
            })
    ...
    {
        return $model->newQuery()->where('kategori_id', CategoryEnum::GALLERY);
    }
