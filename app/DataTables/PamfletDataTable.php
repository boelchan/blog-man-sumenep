...
class PamfletDataTable extends DataTable
{
    public function dataTable($query)
    {
        ...
        return $dataTable->editColumn('published_at', function ($query) {
            if ($query->publish == 'ya') {
                return '<span class="badge bg-success-lt mb-1">Publish '.Carbon::parse($query->published_at)->diffForHumans().'</span> ';
            } else {
                return '<span class="badge bg-secondary-lt">Pending</span>';
            }
        })
            ->editColumn('action', function ($query) {
                return view('components.button.show', ['action' => route('post.pamflet.show', [$query->id, 'uuid' => $query->uuid])]).
                    view('components.button.edit', ['action' => route('post.pamflet.edit', [$query->id, 'uuid' => $query->uuid])]).
                    view('components.button.destroy', ['action' => route('post.pamflet.destroy', [$query->id, 'uuid' => $query->uuid]), 'label' => $query->judul, 'target' => 'pamflet-table']);
            })
    ...
    {
        return $model->newQuery()->where('kategori_id', CategoryEnum::PAMFLET);
    }

    public function html()
    {
        return $this->builder()
            ->setTableId('pamflet-table')
